...
const Alert = ({ open, onHandlerClose }) => {
  return (
    <Transition.Root show={open} as={Fragment}>
      <Dialog as="div" className="relative z-10" onClose={onHandlerClose}>
        <Transition.Child
          as={Fragment}
...
const Pacientes = ({ urlApi, pacientes }) => {
  ...
  const onSubmit = async (values) => {
    console.log(values);
    // const req = await fetch(`${urlApi}/post-paciente`, {
    //   method: "POST",
    ...
    // setTimeout(() => setOpenAlert(false), 10000)
  ...
  return (
    <>
      <Alert open={openAlert} onHandlerClose={() => setOpenAlert(false)} />
      <Navbar />
      <div className="space-y-10">
        ...
        <Transition.Root show={open} as={Fragment}>
          <Dialog as="div" className="relative z-10" onClose={abrirFormulario}>
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0"
...
export const getServerSideProps = async ({ req, query }) => {
  ...
  const pacientes = request || [];

  return {
    props: { urlApi, pacientes },
  };
};
