...
import { getProfesionales, postProfesional, putProfesional, tiposIdProf, sexos, profesion, getDescripProf } from "@/lib"
...
const Pacientes = ({ urlApi, dataProfesionales }) => {
  const [open, setOpen] = useState(false);
  const [openAlert, setOpenAlert] = useState(false);
  const [edit, setEdit] = useState(false);
  const [currentProfesional, setCurrentProfesional] = useState(null);
  const [profesionales, setProfesionales] = useState(dataProfesionales || [])
  ...
  const onSubmit = async (values) => {
    const req = edit ? await putProfesional(values, urlApi) : await postProfesional(values, urlApi)

    console.log("putProfesional", req)

    setDataAlert(req)
    setOpenAlert(!openAlert);

    if (req.tipo == "success") {
      const profesionales = await getProfesionales(urlApi)
              ...
              <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                >
                  <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6">
                    ...
                    <Formik
                      initialValues={edit ? currentProfesional : initialValues}
...
export const getServerSideProps = async ({ req, query }) => {
  ...
  const dataProfesionales = await getProfesionales(urlApi) || [];

  return {
    props: { urlApi, dataProfesionales },
  };
};
